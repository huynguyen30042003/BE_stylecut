...
const refreshToken = [
    ...
    asyncHandler(async (req, res) => {
        ...
        try {
            ...
            await user.save();
            res.json({ accessToken: generateTokenResult.accessToken, refreshToken: generateTokenResult.refreshToken });
        } catch (error) {
            console.error(error)
            return res.status(401).json({ message: 'Invalid refresh token' });
        }
...
const forgotPassword = [
    ...
    asyncHandler(async (req, res) => {
        ...
        const resetUrl = `${req.protocol}://${req.get('host')}/api/auth/reset-password/${forgetPassToken}`;
...
module.exports = { registerUser, authUser, refreshToken, forgotPassword, resetPassword, logoutUser };