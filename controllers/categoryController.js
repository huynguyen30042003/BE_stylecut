...
const updateCategory = [
  ...
  asyncHandler(async (req, res) => {
    ...
    if (updatedCategory) {
      ...
      updatedCategory.description = description || updatedCategory.description;
      updatedCategory.services = services || updatedCategory.services;
      updatedCategory.status = status || updatedCategory.status;

      res.json(await updatedCategory.save());
    } else {
      res.status(404).json({ message: "Category not found" });
    }
  }),
